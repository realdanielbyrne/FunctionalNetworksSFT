...
class TestLayerSpecificationParsing(unittest.TestCase):
    """Test the ICAMask.parse_layer_specification method."""

    def test_single_layer(self):
        """Test single layer specification."""
        ica_mask = ICAMask()
        result = ica_mask.parse_layer_specification("0", 10)
        self.assertEqual(result, [0])

        result = ica_mask.parse_layer_specification("5", 10)
        self.assertEqual(result, [5])

        result = ica_mask.parse_layer_specification("9", 10)
        self.assertEqual(result, [9])

    def test_multiple_individual_layers(self):
        """Test multiple individual layers specification."""
        ica_mask = ICAMask()
        result = ica_mask.parse_layer_specification("0,3,7", 10)
        self.assertEqual(result, [0, 3, 7])

        result = ica_mask.parse_layer_specification("1,2,5,8", 10)
    ...
    def test_range_specifications(self):
        ...
        ica_mask = ICAMask()
        # Basic range
        result = ica_mask.parse_layer_specification("0:4", 10)
        self.assertEqual(result, [0, 1, 2, 3])

        # Range from start
        result = ica_mask.parse_layer_specification(":3", 10)
        self.assertEqual(result, [0, 1, 2])

        # Range to end
        result = ica_mask.parse_layer_specification("7:", 10)
        self.assertEqual(result, [7, 8, 9])

        # Single element range
        result = ica_mask.parse_layer_specification("5:6", 10)
        self.assertEqual(result, [5])

    def test_mixed_specifications(self):
        """Test mixed individual and range specifications."""
        ica_mask = ICAMask()
        result = ica_mask.parse_layer_specification("0,2:5,8", 10)
        self.assertEqual(result, [0, 2, 3, 4, 8])

        result = ica_mask.parse_layer_specification("1,3:6,7,9:", 12)
    ...
    def test_edge_cases(self):
        ...
        result = ica_mask.parse_layer_specification(":", 5)
        self.assertEqual(result, [0, 1, 2, 3, 4])

        # Last layer only
        result = ica_mask.parse_layer_specification("4", 5)
        self.assertEqual(result, [4])

        # Range to last layer
        result = ica_mask.parse_layer_specification("3:", 5)
        self.assertEqual(result, [3, 4])

    def test_duplicate_removal(self):
        """Test that duplicate layer indices are removed."""
        ica_mask = ICAMask()
        result = ica_mask.parse_layer_specification("0,1,0:3,2", 10)
        self.assertEqual(result, [0, 1, 2])

        result = ica_mask.parse_layer_specification("5,3:7,6", 10)
    ...
    def test_error_cases(self):
        ...
        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("", 10)
        ...
        # Negative indices
        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("-1", 10)
        self.assertIn("cannot be negative", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("-1:3", 10)
        self.assertIn("cannot be negative", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("2:-1", 10)
        self.assertIn("cannot be negative", str(cm.exception))

        # Out of bounds indices
        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("10", 10)
        self.assertIn("exceeds total layers", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("5:15", 10)
        self.assertIn("exceeds total layers", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("15:", 10)
        self.assertIn("exceeds total layers", str(cm.exception))

        # Invalid range (start >= end)
        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("5:3", 10)
        self.assertIn("Invalid range", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("5:5", 10)
        self.assertIn("Invalid range", str(cm.exception))

        # Invalid format
        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("abc", 10)
        self.assertIn("Invalid layer index", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("1:2:3", 10)
        self.assertIn("Invalid range format", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            ica_mask.parse_layer_specification("a:b", 10)
        self.assertIn("Invalid range format", str(cm.exception))
    ...
    def test_whitespace_handling(self):
        ...
        result = ica_mask.parse_layer_specification(" 0:4 , 7: ", 10)
        self.assertEqual(result, [0, 1, 2, 3, 7, 8, 9])

        result = ica_mask.parse_layer_specification("  :3  ,  5:  ", 10)
...
class TestICAMaskLayersIntegration(unittest.TestCase):
    ...
    def test_target_layers_parameter_exists(self):
        """Test that target_layers parameter exists in ICAMask.compute_masks_for_model signature."""
        import inspect

        # Verify the method signature includes target_layers parameter
        sig = inspect.signature(ICAMask.compute_masks_for_model)
    ...
    def test_backward_compatibility(self):
        ...
        # This is tested by ensuring the method signature accepts None as default
        import inspect
...
class TestCommandLineIntegration(unittest.TestCase):
    ...
    def test_argument_parser_includes_ica_mask_layers(self):
        ...
        # but we can verify the parameter exists in the ICAMask method signature
        import inspect

        # Verify the parsing method exists and has correct signature
        sig = inspect.signature(ICAMask.parse_layer_specification)
...
class TestErrorHandlingScenarios(unittest.TestCase):
        ...
        for spec in empty_specs:
            with self.subTest(spec=repr(spec)):
                with self.assertRaises(ValueError):
                    ica_mask.parse_layer_specification(spec, 10)
        ...
        for spec in malformed_ranges:
            with self.subTest(spec=spec):
                if spec == "1:":  # This should actually work
                    result = ica_mask.parse_layer_specification(spec, 5)
    ...
    def test_boundary_conditions(self):
        ...
        result = ica_mask.parse_layer_specification("0", 1)
        self.assertEqual(result, [0])

        # Test with single layer model - invalid specs
        with self.assertRaises(ValueError):
            ica_mask.parse_layer_specification("1", 1)  # Out of bounds

        with self.assertRaises(ValueError):
            ica_mask.parse_layer_specification("0:2", 1)  # Range exceeds bounds
    ...
    def test_large_model_specifications(self):
        ...
        # Test large range
        result = ica_mask.parse_layer_specification("90:", large_model_layers)
        expected = list(range(90, 100))
        self.assertEqual(result, expected)

        # Test mixed specification with large numbers
        result = ica_mask.parse_layer_specification("0,50:55,99", large_model_layers)
