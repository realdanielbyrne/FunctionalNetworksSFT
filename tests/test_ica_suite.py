...
def run_ica_tests():
    ...
    # Import test modules
    try:
        from tests.test_ica_masking import TestICAMasking, TestICAMaskComputation, TestICAMaskingIntegration
        from tests.test_ica_cli_integration import TestICACommandLineArguments, TestICATrainingIntegration, TestICAErrorHandling
        from tests.test_ica_mask_application import TestMaskApplication, TestMaskHookBehavior
...
def test_basic_functionality():
    ...
    try:
        # Test imports
        from src.functionalnetworkssft.sft_trainer import apply_ica_masks, compute_ica_masks_for_model
        ...
        class SimpleModel(nn.Module):
            def __init__(self):
                ...
                self.model.layers = nn.ModuleList([
                    nn.Linear(768, 3072),  # This won't match the pattern, but that's OK for basic test
                ])
...
def main():
    ...
    if not test_basic_functionality():
        logger.error("Basic functionality test failed. Cannot proceed with full test suite.")
        return 1
